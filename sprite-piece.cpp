...
void SpritePiece::draw(QPainter &painter, const TileManager &tile_manager, const int starting_palette_line, const int x_offset, const int y_offset) const
{
	...
	for (int tile_x = 0; tile_x < width; ++tile_x)
	{
		...
		for (int tile_y = 0; tile_y < height; ++tile_y)
		{
			...
			painter.drawPixmap(rect, tile_manager.pixmaps(current_tile_index++, (starting_palette_line + palette_line) % Palette::TOTAL_LINES).transformed(flip_transform), QRectF(0, 0, Tile::WIDTH, Tile::HEIGHT));
		}
	}
}
